## Hi, I'm Vita Koval 👋

### Junior Front-end Developer | Scrum Master | React | Next.js

I am a results-oriented Front-end Developer based in **Grimsby, UK**. I specialize in building clean, modern, and high-performance web applications using **JavaScript, React, and Next.js**. With experience as a **Scrum Master**, I bring strong coordination and workflow management skills to every team project.

Currently expanding my expertise into **Node.js & Backend development** to build full-cycle web solutions.

---

### 🧰 Tech Stack
**Frontend:** JavaScript (ES6+) | React | Next.js | HTML5 | CSS3 (SASS/BEM)  
**Backend:** Node.js (Learning) | Express  
**Tools:** Git | GitHub | VS Code | Agile/Scrum | Render | Vercel|Postman

---

### 💻 Featured Projects

#### [Mebleria (2025)](https://mykhailenko-oleksandr.github.io/code-ua-furniture/) — Team Project
*Responsive web application simulating an online furniture store.*
- **Role:** Scrum Master & Developer
- **Key Contributions:**
  - Managed project workflow and team coordination, ensuring timely sprint completion.
  - Developed the **Footer** and implemented the **Product Details modal window**.
  - Responsible for final build, deployment, and performance optimization.
- **Tech Stack:** HTML, CSS, JS, Responsive Design

#### [WatchCharm (2025)](https://oleg-butenko.github.io/code-ua-project/) — Team Project
*Modern responsive website with a focus on visual effects and smooth UI.*
- **Role:** Developer
- **Key Contributions:**
  - Developed the **Hero section**, ensuring 100% compliance with design specifications.
  - Implemented responsive layout principles for a seamless experience across all devices.
  - Focused on interactive elements and CSS transform effects.
- **Tech Stack:** HTML, SASS, JS

---

### 🗣️ Languages
![Ukrainian](https://img.shields.io/badge/Ukrainian-Native-yellow?style=flat-square)
![English](https://img.shields.io/badge/English-B1%2FIntermediate-blue?style=flat-square)

---

### 📫 How to reach me:
📍 **Location:** Grimsby, United Kingdom  
📧 **Email:** [vitakovall2710@gmail.com](mailto:vitakovall2710@gmail.com)  
🔗 **LinkedIn:** [Your LinkedIn Profile](https://linkedin.com/in/vita-koval/)

---
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=VitaKoval27&show_icons=true&theme=radical)
